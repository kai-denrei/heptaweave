...
console.log('ring stave');
{
  ...
  ok('0 is the bare ring', zero.paths.length === 1 && zero.paths[0].place === 'stave');
  ...
  const { ringEnds } = await import('../src/cistercian/ringStave.js');
  const ends = ringEnds();
  const eight = ringStaveGrowthPx({ number: 12345678, size: 300, slots: 8 });
  const ringPath = eight.paths.find(p => p.place === 'stave');
  const cxy = 150;
  ...
  ok('the growth front starts at the thin end', ringPath.points[0].d === 0);
