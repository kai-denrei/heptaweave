...
self.addEventListener('fetch', (event) => {
  ...
  } else if (req.destination === 'script' || url.pathname.endsWith('.js')) {
    strategy = () => staleWhileRevalidate(req);
  } else if (req.destination === 'image' || url.pathname.endsWith('.webmanifest') || url.pathname.endsWith('.svg')) {
    strategy = () => cacheFirst(req);
  }
  if (!strategy) return;
